...
def unwire_if_dead(certdir: Path) -> bool:
    ...
    # recycle does it twice — measured on lmd42 2026-08-18, 70 seconds apart.
...
def _wire_global_config_locked(
    ...
    if port is None or ca_path is None:
        ...
        #   lambda-docker partly because the ambient store is a capath with no
...
# via-personal-mac, same proxy and same certificate, three verifiers:
#
...
# clock skew either side. Measured on via-personal-mac by generating a cert and
...
def live_bridge_names() -> dict[str, str]:
    ...
    and six 'lambda-docker-<word>-<word>', for sessions that all had names.
...
def server_generated_titles() -> set[str]:
    ...
    they rename a session to `lambda-docker-myproject`, no regex can tell it
...
def _looks_generated(title: str) -> bool:
    ...
    has no name at all (`lambda-docker-cozy-badger`) is minted SERVER-SIDE and
    ...
    prefixes: `lambda-docker-…`, `seca2033000822-…`, `mbpm1p-saic-…`. Derived
...
def _pin_daemon_pids(certdir: Path) -> list[int]:
    ...
    for line in out.splitlines():
        ...
        #
        # Measured on via-work-mac, found in production rather than here: the
...
def _sweep_orphan_daemons(certdir: Path, keep_pid: int) -> None:
    ...
    # Measured on lmd42 2026-08-18 and it does not: a draining predecessor's
    ...
    # as twelve still being written — measured on lmd42 2026-08-18, where a
...
def announce_draining(certdir: Path, pid: int | None = None):
    ...
    Measured on lmd42 2026-08-18, the 0.1.100 rollout:
...
def beat_draining(certdir: Path, pid: int | None = None,
    ...
    try:
        ...
        # line — measured on lmd42 2026-08-18, pid 609285 held twelve live
...
#
#     lmd42  cut 16   (16 mid-response, 0 before headers)
...
class PortHolder:
    ...
    def _supervise(self) -> None:
        while not self._stop:
            ...
            # on via-work-mac — a zombie standby read as alive to three
...
def _retire_stale_standbys(certdir, keep_pid: int | None = None) -> int:
    ...
    this whole design covers. Every such death adds one. Measured on lmd42:
...
def _standby_port_still_wanted(certdir, port: int) -> bool:
    ...
    STOPS A RUNAWAY, measured on lmd42. Every deploy hands the daemon to a new
...
def _claim_arm(certdir) -> "int | None":
    ...
    silent window arms ALL of them at once. Measured on lmd42: three armed
...
def teardown_drain_budget(reason: str, held: bool,
    ...
    A FOURTH EXIT PATH, AND THE ONLY ONE STILL CUTTING. Measured on lmd42, the
    ...
    #
    # MEASURED ON via-work-mac 2026-08-18, four replies:
...
def wire_env(
    ...
    # lambda-docker is the statusline nudge -- usage.py `Popen([cswap,
...
class PinProxy:
    ...
    def await_inflight(self, budget: float) -> int:
        ...
        #
        # Measured on lmd42 2026-08-18: one code change produced two complete
    ...
    def restore_titles_after_connect(self, token: str) -> int:
        ...
        `lambda-docker-serene-unicorn` from then on. Every piece was in place:
        ...
        with self._live_lock:
            if conn in self._owed:
                ...
                if content:
                    ...
                    # DURATION. Measured on lmd42: pid 1269189 started 19:31,
    ...
    def _restore_bridge_titles(self, sessions: list[dict], token: str) -> int:
        ...
        'Session interrupted by user' twice, six 'lambda-docker-<word>-<word>',
        and a reconnect on another machine produced 'seca2033000822-curious-
    ...
    def direct_last(self) -> "float | None":
        ...
        Measured on via-work-mac 2026-08-06 — 9901 stopped tunnelling at
    ...
    def _serve_health(self, conn: socket.socket) -> None:
        ...
        # on lmd42 for one day: 61 DIRECT transitions, 148 dial-failed,
    ...
    def _note_egress(
        ...
        *,
        ...
        if direct and not configured:
            ...
            # for DIRECT and never generalised. Measured on lambda-docker:
...
# rate or frame width has to be guessed. Measured on lmd42 2026-08-18 — twelve
...
def _relay_response(
    ...
    if keep and not interim and this_process_is_draining():
        ...
        # socket — measured on lmd42 2026-08-18, eleven sessions whose ONLY
